...
;(function($) {
    ...
    $.BigVideo = function(options) {
		...
		function updateSize() {
			...
			if (currMediaType === 'video') {
				...
				$(vidEl+'_html5_api').css('width','1024px').css('height','576px');
				$(vidEl+'_flash_api').css('width','1024px');
			} else {
				// is image
				$('#big-video-image')
					.width(1024)
		...
		function nextMedia() {
			currMediaIndex++;
			config.currentNewsOrder = currMediaIndex;
			if (currMediaIndex === playlist.length){
				currMediaIndex=0;
			...
			playVideo(playlist[currMediaIndex]);
        		...
        		function prevMedia(){
        			...
        			config.currentNewsOrder = currMediaIndex;
        			if(currMediaIndex === -1){
        				currMediaIndex = playlist.length-1;
        				config.currentNewsOrder = playlist.length-1;
        			}
        			playVideo(playlist[currMediaIndex]);
        		}
	...
	BigVideo.init = function() {
        ...
        BigVideo.next = function(){
        	nextMedia();
        };

        BigVideo.prev = function(){
        	prevMedia();
        };
